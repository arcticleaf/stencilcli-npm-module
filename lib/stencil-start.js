...
class StencilStart {
    ...
    async run(cliOptions) {
        ...
        if (cliOptions.disableBS === true && this._buildConfigManger.production) {
            this._buildConfigManger.initWorker().production(() => {
                if (cliOptions.displayShippyPath === true) {
                    this._logger.log(`{"shippy": {"url":"http://localhost:${initialStencilConfig.port}"}}`);
                }
            });
        } else {
